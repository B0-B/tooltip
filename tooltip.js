var toolTipHandler = async function () {
    // ---- globals ----
    ...
    console.log('run rooltip listener')
    ...
    while (true) {
        ...
        tags.forEach(tag => {
            ...
            const grandParentNode = tag.parentNode;
            grandParentNode.removeChild(tag);
            newTag = el('tt'); grandParentNode.appendChild(newTag);

            // loop through oldScript attributes and apply them on new script
            console.log('attrubutes', attributes);
            newTag.style.width = tag.style.width;
            newTag.style.height = tag.style.height;
            tag = newTag;
            //Object.keys(attributes).forEach(k => {console.log(k, attributes[k]); tag.setAttribute(k, attributes[k])});
